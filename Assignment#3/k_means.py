'''
This script is the implementation of k_means 
algorithm and the 2D data is plotted in three clusters
'''
...
def plot_data(X, centeroids, clusters):
    ...
    for i in range(len(X)):
        plt.scatter(X[i, 0], X[i,1], marker= markers[int(clusters[i])], s=7, color = colors[int(clusters[i])])
...
def k_means(X, k=3):
    ...
    # picking the futhest distanced centers
    # A = np.random.uniform(np.min(X), np.max(X), size=(1, d))
    A = X[np.random.randint(0,199)] 
    for i in range(n):
        temp = dist(X[i], A)
        if temp > max_dist:
            max_dist = temp
